...
const todos = JSON.parse(localStorage.getItem("disney"));
if (todos) {
  todos.forEach((todo) => {
    add(todo);
  });
...
function saveData() {
  ...
  localStorage.setItem("disney", JSON.stringify(todos));
}
...
function test() {
  ...
  let replyText = localStorage.setItem("disney-post", reply);
